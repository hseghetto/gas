...
#a=pd.read_csv("gas_terceiro_caso_interpolado.txt",sep=" ",usecols=[0,1,2])
...
plt.scatter([i[0] for i in time[0:len(predict_data)]],[j[0][1]/300 for j in predict_data[0:len(predict_data)]],c=[k[0] for k in time[0:len(predict_data)]],cmap="Set1")
plt.xlabel("Time")
plt.ylabel("Presssure")
#plt.ylim([100,300])
plt.show()
