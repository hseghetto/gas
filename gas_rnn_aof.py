...
for seed in range(0,3): 
    ...
    train_split=int(len(train_data)*0.5)
    ...
    model.add(keras.layers.GRU(layer_size, kernel_regularizer=keras.regularizers.l1_l2(reg,0.00),
                     activity_regularizer=keras.regularizers.l1(0.), input_shape=(train_data_norm.shape[1], train_data_norm.shape[2])))
    ...
    #model.add(keras.layers.Dense(layer_size,activation="tanh"))    
    model.add(keras.layers.Dense(1,kernel_regularizer=keras.regularizers.l1_l2(reg,0.0),
    ...
    history = model.fit(train_data_norm[train_index], target[train_index], epochs=EPOCHS,
                        ...
                        verbose=0, callbacks=[PrintDot(),early_stop],batch_size=8)
    ...
    a=a[index[2]-last:index[4]] #use index-last: to predict from the last point before 144h 
